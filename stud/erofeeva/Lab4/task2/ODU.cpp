...
vect shooting_solve(double a, double b, double alpha, double beta, double y0, double delta, double gamma, double y1, double h, double eps) { 
    double eta_prev = 0.9;
    double eta = 0.8;
    while (1) {
        ...
        double eta_next = eta - (eta - eta_prev) / (phi - phi_prev) * phi;
...
int main() {
    ...
    double a = 0, b = PI/6;
    double alpha = 1, beta = 0, y0 = 0;
    double delta = 1, gamma = 0, y1 = -sqrt(3)/3;
    ...
    for (int i = 0; i < sol_shooting_h1.size(); ++i) {
        double ex_y = exact_y(get<0>(sol_shooting_h1[i]));
        cout << fixed << setprecision(9) << "x: " << get<0>(sol_shooting_h1[i]) << "  y: " << get<1>(sol_shooting_h1[i]) << "  exact y: " << ex_y << endl;
    }
    cout << "Runge-Rumberg error:" << endl;
    double shooting_err = runge_romberg_max(sol_shooting_h1, sol_shooting_h2, 4);
    cout << shooting_err << endl << endl;
    ...
    for (int i = 0; i < sol_fin_dif_h1.size(); ++i) {
        double ex_y = exact_y(get<0>(sol_fin_dif_h1[i]));
        cout << fixed << setprecision(9) << "x: " << get<0>(sol_fin_dif_h1[i]) << "  y: " << get<1>(sol_fin_dif_h1[i]) << "  exact y: " << ex_y << endl;
    }
    cout << "Runge-Rumberg error:" << endl;
